...
<script type="application/javascript">
    $(function() {
        ...
        var lastClickDeptId = -1; //上次点击的部门id

        var deptListTemplate = $('#deptListTemplate').html();
        Mustache.parse(deptListTemplate);//使用Mustache引擎处理
        ...
        // 绑定部门点击事件
        // 点击部门名称加载列表、编辑部门、删除部门等功能需要等待实际部门渲染完才能使用
        // 比如修改部门一定要等到实际部门出来了有实际数据了才能有修改功能及修改操作
        ...
        function handleDepSelected(deptId) {
            ...
            loadUserList(deptId);//加载被选择部门下的用户列表
        }
	    ...
	    //点击部门添加按钮(新增按钮及功能在初始化完页面就可以支持)
        $(".dept-add").click(function() {
            $("#dialog-dept-form").dialog({
                modal: true,
                title: "新增部门",
                open: function(event, ui) {
                    $(".ui-dialog-titlebar-close", $(this).parent()).hide();//隐藏默认的关闭按钮，所有按钮使用新增的button
                    optionStr = '<option value="0">-</option>';
                    recursiveRenderDeptSelect(deptList, 1);
                    $("#deptForm")[0].reset();
                    $("#parentId").html(optionStr);
