...
<script type="text/javascript">
    $(function () {
        ...
        var lastRoleId = -1; //上次点击的角色列表id
        var selectFirstTab = true; //被选择的tab  角色权限、角色用户
        var hasMultiSelect = false; //是否有多选
        
        var roleListTemplate = $("#roleListTemplate").html();
        Mustache.parse(roleListTemplate);//使用Mustache引擎处理
        var selectedUsersTemplate = $("#selectedUsersTemplate").html();//当前选择角色 选中用户的模板
        Mustache.parse(selectedUsersTemplate);
        var unSelectedUsersTemplate = $("#unSelectedUsersTemplate").html();//当前选择角色 未选中用户的模板
        ...
        function bindRoleClick() {
            ...
            $(".role-name").click(function (e) {
               ...
               e.stopPropagation();
               var roleId = $(this).attr("data-id");
               handleRoleSelected(roleId);//加载被选择的角色名称下的角色权限和角色用户关系
            });
        }

        //加载被选择的角色名称下的角色权限和角色用户关系
        function handleRoleSelected(roleId) {
            ...
            if (selectFirstTab) {
                loadRoleAcl(roleId); //加载被选中角色名称下的角色权限关系列表
            }
        }
        ...
        function renderRoleTree(aclModuleList) {
            zTreeObj = [];
            recursivePrepareTreeData(aclModuleList);//递归生成层级树结构
